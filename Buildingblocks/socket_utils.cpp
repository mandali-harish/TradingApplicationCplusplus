...
namespace Common 
{
    ...
    {
        char buf[NI_MAXHOST] = {'\0'}; //buffer to store the Interface IP
        ...
        if(getifaddrs(&ifaddr) != -1)
        {
            for(ifaddrs *ifa = ifaddr; ifa; ifa= ifa->ifa_next)
            {
                ...
                {
                    getnameinfo(ifa->ifa_addr, sizeof(sockaddr_in), buf, sizeof(buf), NULL, 0, NI_NUMERICHOST); //stores the interface IP in the buffer
                    break;
                }
            }
            freeifaddrs(ifaddr);
    ...
    auto setNonBlocking(int fd) -> bool
    {
        ...
        if(flags & O_NONBLOCK)
            return true;
        return (fcntl(fd, F_SETFL, flags | O_NONBLOCK) != -1); //returns true if the socket file descriptor was already non-blocking or the method was able to successfully make it non-blocking
    }
    ...
    {
        std::string time_str;
        const auto ip = t_ip.empty() ? Common::getIfaceIP(iface) : t_ip;
        ...
        if(rc)
        {
            logger.log("getaddrinfo() failed. error:% errno:%n", gai_strerror(rc), strerror(errno));
            return -1;
        }
        ...
        for(addrinfo *rp = result; rp; rp = rp->ai_next)
        {
            ...
            if(!is_blocking)
            {
                ...
                {
                    logger.log ("setNonDelay() failed. errno:%\n", strerror(errno));
                    return -1;
                }
            }
            ...
            if(!is_udp && is_listening && listen(fd, Common::MaxTCPServerBacklog) == -1)
            {
